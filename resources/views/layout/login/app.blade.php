...
<form action="" method="POST" role="form" class="col-md-8">
  ...
  <div class="form-group {{ $errors->first('name') ? ' has-error':'' }} {{ isset($reset) ? 'd-none' :'' }} {{ isset($login) ?'d-none' : ''}} "  >
    ...
    <input type="text" name ="name" class="form-control" id="exampleInputEmail1" value="{{ isset($login->name) ?? old('name','')  }}" autocomplete="off">
  ...
  <div class="form-group {{ isset($reset) ? 'd-none' :'' }} {{ $errors->first('password') ? ' has-error':'' }}"  >
    ...
    <input type="password" name ="password" value="123456789" class="form-control" id="xampleInputEmai1"  autocomplete="off">
...
<a class="{{ isset($login) ? 'd-none' :''}}" style="color:blue;text-decoration:underline;font-size:18px;line-height:24px" href="{{route('get.login') }}">Đăng nhập</a><br>
<a class="{{ isset($register) ? 'd-none' :''}}" style="color:blue;text-decoration:underline;font-size:18px;line-height:24px" href="{{route('get.register') }}">Đăng ký</a><br>
<a class="{{ isset($reset) ? 'd-none' :''}}" style="color:blue;text-decoration:underline;font-size:18px;line-height:24px" href="{{route('get.forgot-password')}}">Quên mật khẩu</a><br>
</div>
